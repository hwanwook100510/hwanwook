      ...
      <section className="design-wide suggestion-layout">
        <div>
          <article id="suggestion-form" className="design-card suggestion-form">
            ...
            {!user ? <><p>로그인 후 정책 제안을 제출할 수 있습니다.</p><button className="design-primary" type="button" onClick={loginWithGoogle}>Google로 로그인</button></> : isAdmin ? <p>관리자는 정책 제안을 제출할 수 없습니다. 관리자 페이지에서 조회만 가능합니다.</p> : !profile ? <><p>학생 정보 등록 후 정책 제안을 제출할 수 있습니다.</p><Link className="design-primary" to="/register">회원가입 하러 가기</Link></> : <div className="suggestion-form-fields"><label className="form-group"><span>제목</span><input maxLength={80} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} /></label><label className="form-group"><span>분야</span><select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })}>{categories.map((category) => <option key={category}>{category}</option>)}</select></label><label className="form-group text-area-group"><span>내용</span><textarea maxLength={1000} rows={5} value={form.content} onChange={(event) => setForm({ ...form, content: event.target.value })} /></label><label className="form-group text-area-group"><span>기대 효과</span><textarea maxLength={500} rows={3} value={form.effect} onChange={(event) => setForm({ ...form, effect: event.target.value })} /></label><label className="select-pill suggestion-anonymous"><input type="checkbox" checked={form.isAnonymous} onChange={(event) => setForm({ ...form, isAnonymous: event.target.checked })} /><span className="anonymous-label">공개 목록에서 익명 표시</span></label><button className="design-primary suggestion-submit" type="button" onClick={submitSuggestion}>정책 제안 제출</button></div>}
          </article>
