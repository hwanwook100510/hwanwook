...
function formatDate(value: EvaluationResponse['createdAt']) {
...
function Evaluation() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!user?.email) { setMessage('이 기능은 로그인 후 사용할 수 있습니다.'); return }
    if (!profile) { setMessage('회원가입 정보를 확인할 수 없습니다.'); return }
    if (!db) { setMessage('DB에 연결할 수 없어 평가를 저장하지 못했습니다.'); return }

    const previousSummary = summaryResult
    const previousResponses = responses
    const response: EvaluationResponse = { email: user.email, name: profile.name, createdAt: serverTimestamp() as unknown as EvaluationResponse['createdAt'], ...vote }
    ...
    try {
      await setDoc(doc(db, 'evaluationResponses', user.email), response, { merge: true })
      setMessage('평가가 반영되었습니다.')
    } catch {
      setSummaryResult(previousSummary)
  ...
  return (
    <div className="design-page">
      ...
      {isAdmin && <section className="design-wide design-card"><h2>개인별 평가 응답</h2>{responses.length === 0 ? <p>아직 평가 응답이 없습니다.</p> : responses.map((response) => <p key={response.email}>{response.name} · {response.email} · {formatDate(response.createdAt)}</p>)}</section>}
